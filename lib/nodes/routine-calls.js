...
module.exports = function (node) {
    ...
    node.ProcedureCall.prototype.interpret = function (context) {
        ...
        if (!target.declaration) {
            throw new node.errors.InterpreterException('El procedimiento ' + this.name + ' no se encuentra definido.', this, {code: 'undefined_procedure', detail: {name: this.name}});
        }
    ...
    node.FunctionCall.prototype.eval = function (context, options) {
        var target = this.declarationProvider();
        if (!target.declaration) {
            throw new node.errors.InterpreterException('La función "' + this.name + '" no se encuentra definida.', this.token, {code: 'undefined_function', detail: {name: this.name}});
        }
        var declaration = target.declaration;
        var parameterValues = evalArguments(this.token, context, this.parameters, {});
        context.startContext(this.name);
